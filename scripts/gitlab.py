...
class GitlabBackup:
    def __init__(self, auth_token, group_id, api_url):
            ...
            for index in range(len(data)):
                for key in data[index]:
                    if key == 'http_url_to_repo':
                        group_projects[0].append(data[index][key])
                    if key == 'path_with_namespace':
                        group_projects[1].append(data[index][key].split('/', 1))
    ...
    def backup_group_repositories(self, directory_path, group_projects_arr):
        ...
        try:
            for i in range(len(group_projects_arr[0])):
                repo_name = group_projects_arr[1][i][1]
                repo_url = group_projects_arr[0][i]
                file_path = os.path.join(directory_path, repo_name)
                ...
                if not path_exists:
                    os.chdir(directory_path)
                    git.Git().clone(self.clone_base_url + repo_url.split("https://gitlab.com/")[1],
                                    os.path.join(directory_path, repo_name))
